...
CELL_GROUP_TYPES=${CELL_GROUP_TYPES:-"inferred cell type,authors inferred cell type,inferred cell type - ontology labels,inferred cell type - authors labels,authors cell type - ontology labels,authors cell type"}
...
tail -n +2 $clustersToLoad | sed s/\"//g | awk -F',' 'BEGIN { OFS = "|"; } {print $1,$2,$3,$4}' > $groupMembershipsToLoad
...
if [ -n "$CONDENSED_SDRF_TSV" ]; then
  ...
  for additionalCellGroupType in "${additionalCellGroupTypes[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        grep "$(printf '\t')$additionalCellGroupType$(printf '\t')" $CONDENSED_SDRF_TSV | awk -F'\t' 'BEGIN { OFS = "|"; } {print $1,$3,$5,$6}' | sort | uniq | sed 's/ *$//' | sed 's/|$/|Not available/g' >> $groupMembershipsToLoad    
