...
writeMM(exp_matrix,file="TEST-EXP.expression_tpm.mtx")
system(paste0("gzip -f ","TEST-EXP.expression_tpm.mtx"))
write.tsv(data.frame(list(ids=seq(1,nrow(exp_matrix)),lab=rownames(exp_matrix))),
          header=FALSE,rownames.label=NULL,fix=FALSE,gzip=TRUE,file="TEST-EXP.expression_tpm.mtx_rows.gz")

write.tsv(data.frame(list(ids=seq(1,ncol(exp_matrix)),lab=colnames(exp_matrix))),
          header=FALSE,rownames.label=NULL,fix=FALSE,gzip=TRUE,file="TEST-EXP.expression_tpm.mtx_cols.gz")

# Write query result file to compare against.
whereClause<-paste(sprintf("(gene_id = '%s' AND cell_id = '%s')",
                           rownames(exp_matrix)[genes_i],
                           colnames(exp_matrix)[runs_j]), collapse = " OR ")
...
fileConn<-file("query_test.sql")
writeLines(query, fileConn)
close(fileConn)

query_result<-data.frame(experiment_accession=rep("TEST-EXP",300),
                        gene_id=rownames(exp_matrix)[genes_i],
                        cell_id=colnames(exp_matrix)[runs_j],
                        expression_level=exp)
attach(query_result)
write.table(query_result[order(gene_id, cell_id),], file="query_expected.txt", quote = FALSE, row.names = FALSE)
